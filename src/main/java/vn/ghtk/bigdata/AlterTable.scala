...
import vn.ghtk.bigdata.analysis.{AddColumns, MysqlAnalysisHelper, RenameColumn}
...
object AlterTable {
  def main(args: Array[String]): Unit = {
    ...
    val eventTimestamp = arguments("eventTimestamp").toLong
    val jdbcType = arguments("jdbcType")
    ...
    val execute = new MysqlAnalysisHelper().resolveOperatorsUp(stm) match {
      case cmd: AddColumns => AlterTableAddColumnsExecute(cmd, eventTimestamp)
      case cmd: RenameColumn => AlterTableRenameColumnsExecute()
    }
